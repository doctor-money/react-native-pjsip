...
public class PjSipService extends Service {
    ...
    public void onDestroy() {
        Log.w(TAG, "onDestroy");
          try {
            ...
            PjSipAccount a = new PjSipAccount(this, 999999, c);
             getEmitter().fireRegistrationChangeEvent(a);
        } catch (Exception e) {
        ...
        try {
            if (mEndpoint != null) {
                Log.w(TAG, "onDestroy libDestroy");
    ...
    private void handleAccountRegister(Intent intent) {
        try {
            ...
            if (account == null) {
                throw new Exception("handleAccountRegisterAccount with \""+ accountId +"\" id not found");
            }
    ...
    private void handleAccountDelete(Intent intent) {
        try {
            ...
            if (account == null) {
                throw new Exception("handleAccountDelete Account with \""+ accountId +"\" id not found");
            }
