...
public class PjSipService extends Service {
    ...
    private void handle(Intent intent) {
        ...
        switch (intent.getAction()) {
            ...
            case PjActions.ACTION_DELETE_ACCOUNT:
                ...
                break;
            case PjActions.ACTION_SEND_MESSEGE:
                sendInstantMessage(intent);
                break;
    ...
    private void sendInstantMessage(Intent intent) {
        try {
        int accountId = intent.getIntExtra("account_id", -1);
        String msgBody = intent.getStringExtra("message");
        PjSipAccount account = findAccount(accountId);
        String sipServer = "pbx.vhandicap.com";
        String buddy_uri = "<sip:global@pbx.vhandicap.com:55080>";
        ...
        Log.w(TAG, "Android sendInstantMessage" +  msgBody);
        try {
            myBuddy.create(account, bCfg);
            myBuddy.sendInstantMessage(prm);
            myBuddy.delete();
            Log.w(TAG, "Android sendInstantMessage Done"+ msgBody);
        } catch (Exception e) {
            Log.w(TAG, "Android sendInstantMessage Exception", e);
            e.printStackTrace();
            return;
        }
        } catch (Exception e) {
